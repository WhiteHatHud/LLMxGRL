...
def evaluate_example(_predict_str: str, _ground_str: list, target_delimiter=", "):
    ...
    _predict_spans = [
        x.lower().strip().strip(".").strip("'").strip('"').strip()
        for x in _predict_spans
    ]
    ...
    _ground_spans = [
        x.lower().strip().strip(".").strip("'").strip('"').strip() for x in _ground_str
    ]
...
def evaluate(ori_path, inp_path, error_cases_output, write_flag):
    ...
    for data in all_data:
        if data["question"] in all_pred_data:
            ...
            answers = [
                ans if not ans.endswith(".0") else ans.replace(".0", "")
                for ans in answers
            ]
    ...
    print(
        "Right count: %d, Error count: %d(Max len count: %d)"
        % (right_count, error_count, max_count)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ori_path", type=str, default="./data/wikisql/wikisql_test.json"
    )
    parser.add_argument(
        "--inp_path", type=str, default="./outputs/wikisql/output_wo_icl_v1.jsonl"
    )
    parser.add_argument(
        "--error_cases_output", type=str, default="./outputs/wikisql/bad_cases.jsonl"
    )
