...
def evaluate(path, db_dir, etype, kmaps, db_dir2):
    ...
    for p, g in zip(plist, glist):
        ...
        if etype in ["all", "exec"]:
            if db_dir2 == "":
                db2 = db
            results = eval_exec_match(db, db2, p_ori, g_str, p_sql, g_sql)
            if results is False:
                continue
            else:
                exec_score, (pred, gold) = results
                if exec_score == 0:
...
def eval_exec_match(db, db2, p_str, g_str, pred, gold):
    ...
    return result_eq(p_res, q_res, order_matters=orders_matter), (p_res, q_res)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', dest='path', type=str,default="outputs/spider/output.jsonl")
